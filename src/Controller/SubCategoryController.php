...
final class SubCategoryController extends AbstractController
{
    ...
    {
        // hna kanjib toutes les sous-catégories men DB
    ...
    {
        // créer subCategory jdida
        $subCategory = new SubCategory();
        // formulaire dial création
        $form = $this->createForm(SubCategoryType::class, $subCategory);
        $form->handleRequest($request);

        // ila formulaire valid → save f DB
        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager->persist($subCategory);
            $entityManager->flush();

            // redirection vers liste dial sous-catégories
    ...
    {
        // afficher détail d’une sous-catégorie
    ...
    {
        // formulaire dial modification
        $form = $this->createForm(SubCategoryType::class, $subCategory);
        $form->handleRequest($request);

         // ila formulaire valid → flush les modifications
        if ($form->isSubmitted() && $form->isValid()) {
            $entityManager->flush();
    ...
    {
        // vérifier CSRF token avant suppression
